...
이 문서는 4인 부트캠프 프로젝트의 통합 테스트와 시연에 사용할 AWS 개발 환경을 정의한다. 운영 환경의 고가용성을 그대로 복제하지 않고, EKS와 Application Load Balancer(ALB)의 필수 다중 AZ 조건은 지키면서 실제 유료 워크로드는 한 AZ에 집중한다.

현재 범위는 다음과 같다.

- 별도 AWS Dev Account를 만들지 않고 현재 사용하는 한 AWS Account에 구축한다.
- `dev` 환경의 `alpha` Cell만 구축한다.
...
- 운영 환경은 이 개발 환경을 직접 확장하지 않고, 같은 모듈을 별도 환경 설정으로 재사용해 구축한다.
...
이 환경의 구성 원칙은 **2-AZ Control/Network Plane + 1-AZ Dev Workload**다.
...
- `doro-erp-dev-alpha-queue-events.fifo`
- `doro-erp-dev-alpha-audit-events.fifo`
...
Terraform을 직접 실행하는 팀원은 공유 Access Key를 사용하지 않고 자신의 AWS Principal로 인증한다. 로컬 AWS CLI Profile 이름은 `erp-dev`를 사용한다. 인증한 Principal은 같은 Account의 Terraform 실행 Role을 Assume한다. Application 개발·검토만 수행하는 팀원에게는 Terraform Role 접근 권한이 필요하지 않다.

```text
팀원 개인 AWS Principal
  → sts:AssumeRole
  → doro-erp-dev-terraform
  → Dev Terraform 자원 생성·변경
```

- Terraform Role 이름: `doro-erp-dev-terraform`
...
```powershell
aws sts get-caller-identity --profile erp-dev --output json
```
...
`doro-erp-dev-terraform` Role을 관리하는 Terraform 변수에 승인된 팀원의 ARN을 추가한다. IAM Console에서 수동으로 Trust Policy를 수정하지 않는다.
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:iam::727646470302:role/doro-erp-dev-terraform"
    }
  ]
}
```
...
```ini
[profile erp-dev-source]
region = ap-northeast-2
output = json

[profile erp-dev]
role_arn = arn:aws:iam::727646470302:role/doro-erp-dev-terraform
source_profile = erp-dev-source
role_session_name = doro-erp-dev-IAM_USER_NAME
region = ap-northeast-2
output = json
```

- `erp-dev-source`: 팀원 자신의 IAM 인증을 보관하는 로컬 전용 Profile
- `erp-dev`: Terraform Role을 Assume해 임시 Credential을 받는 공통 실행 Profile
...
하나의 Access Key를 여러 팀원이 복사하면 누가 작업했는지 CloudTrail에서 구분하기 어렵고, 한 명의 유출·탈퇴·권한 회수 때문에 모든 팀원의 Key를 동시에 교체해야 한다. 따라서 공유 Access Key가 이미 있더라도 폐기 대상으로 관리하고, 각 팀원의 개인 Principal에서 `doro-erp-dev-terraform` Role을 Assume한다.

현재 개인 인증에 `erp-dev` 이름을 사용 중이라면 먼저 해당 Profile을 `erp-dev-source`로 옮긴 뒤 Role Profile을 `erp-dev`로 구성한다. Profile 변경 전 기존 `%UserProfile%\.aws\config`와 `credentials`는 외부에 공유하지 않고 로컬에서만 백업한다.
...
```text
arn:aws:sts::727646470302:assumed-role/doro-erp-dev-terraform/doro-erp-dev-IAM_USER_NAME
```
...
Argo CD는 Dev 인프라의 Bootstrap·Foundation·Data 자원 생성에 필요한 선행 구성요소가 아니다. EKS, ECR, RDS, SQS, Pod Identity와 Application Manifest가 각각 정상 동작하는지 먼저 검증한 뒤 배포 자동화 단계에서 도입한다.

- Argo CD 도입 전에도 Kubernetes Manifest의 정본은 Git의 Kustomize Base와 Dev Alpha Overlay다.
- 초기 배포는 `kubectl apply -k deploy/overlays/dev/alpha`처럼 승인된 Overlay 단위로 수행한다.
...
- GitHub Actions가 테스트, 불변 Image Push와 Dev Overlay Image Digest 변경을 수행할 수 있다.
...
6. Production 후보 환경은 승인된 Overlay 변경 후 Sync하는 정책을 사용하며 Dev 자동 Sync 정책을 그대로 복사하지 않는다.

Dev 자동 Sync의 목표 정책은 `selfHeal=true`, `prune=true`다. 다만 최초 연결과 삭제 영향 검증 전에는 자동 Sync와 Prune을 활성화하지 않는다.
...
- Dev 자동 Sync·Self Heal·Prune과 Git Revert 검증
...
운영 환경 전환 시 Dev 자원을 직접 수정하거나 승격하지 않는다. Terraform Module과 Kubernetes Base는 재사용하되 별도의 `prod` State와 환경 설정으로 새 자원을 생성한다.
...
Dev 구성의 코드와 배포 규칙을 재사용할 수 있으므로 전환 작업의 중심은 새 자원 생성과 운영 수치 확정이다. 다만 현재의 1-AZ 워크로드를 그대로 운영 환경으로 사용하는 것은 허용하지 않는다.
