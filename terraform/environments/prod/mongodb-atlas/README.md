# Prod Alpha MongoDB Atlas Free Terraform

발표와 개발 통합 테스트에 사용할 Audit Service 전용 MongoDB Atlas 8.0 M0 Free Cluster를 만든다. PrivateLink와 Atlas Cloud Backup/PIT는 사용하지 않으며, Database Network Access에는 Network Terraform이 만든 `team2` NAT Gateway의 고정 공인 IP `/32`만 허용한다.

```text
Audit Pod 또는 SSM 관리 EC2
→ Terraform 관리형 doro-erp-prod-nat-a NAT Gateway
→ TLS
...
Network Stack이 먼저 적용되어 `doro-erp-prod-nat-a` NAT Gateway가 `available` 상태여야 한다.

```bash
cd ~/Doro-ERP-Infra/terraform/environments/prod/mongodb-atlas
...
이 전체 값을 AWS Secrets Manager의 `doro-erp/prod/alpha/audit` Secret에서 `AUDIT_MONGODB_URI` 값으로 저장한다. URI는 Terraform Variable, Output, Git과 Shell History에 남기지 않는다.
...
aws ec2 describe-nat-gateways \
  --filter Name=tag:Name,Values=doro-erp-prod-nat-a Name=state,Values=available \
