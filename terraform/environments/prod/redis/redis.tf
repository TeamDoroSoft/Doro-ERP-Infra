...
resource "aws_vpc_security_group_ingress_rule" "redis_from_eks" {
  ...
  description                  = "Redis TLS from the Prod EKS cluster"
  ip_protocol                  = "tcp"
  from_port                    = 6379
  to_port                      = 6379
  referenced_security_group_id = data.aws_eks_cluster.prod.vpc_config[0].cluster_security_group_id
}
...
resource "aws_elasticache_replication_group" "this" {
  ...
  description          = "Doro ERP Prod Alpha Store Access sessions and login rate limits"
