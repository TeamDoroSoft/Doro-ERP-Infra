...
삭제 범위는 Doro ERP 기존 환경이 소유한 자원이다. Terraform 실행에 필요한 개인 IAM User와
`minseok.click` Domain 등록만 외부 선행조건이다. 팀 운영자 Group, GitHub OIDC Provider와
Route 53 Hosted Zone은 새 Bootstrap State가 생성·관리하므로 Network 전환 중 삭제하지 않는다.
