resource "aws_s3_bucket" "frontend" {
  bucket = "doro-erp-prod-alpha-frontend-${var.aws_account_id}"
}
...
resource "aws_cloudfront_origin_access_control" "frontend" {
  ...
  description                       = "OAC for the private Prod Alpha SPA bucket."
...
resource "aws_security_group" "alpha_alb_frontend" {
  ...
  description = "Allow CloudFront VPC origin traffic to the Prod Alpha internal ALB."
...
resource "aws_wafv2_web_acl" "frontend" {
  ...
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "doro-erp-prod-common-rules"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "doro-erp-prod-alpha"
    sampled_requests_enabled   = true
  }
}
