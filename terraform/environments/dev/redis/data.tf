  ...
  lifecycle {
    ...
    precondition {
      condition = alltrue([
        ...
        contains(data.aws_eks_cluster.dev.vpc_config[0].subnet_ids, "subnet-000e8271451fdf084")
      ])
      error_message = "The configured VPC, data subnets, or EKS cluster no longer match the Dev network contract."
    }
  }
}
