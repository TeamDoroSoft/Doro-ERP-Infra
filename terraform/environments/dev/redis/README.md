# Dev Alpha Redis Terraform

Store Access의 Spring Session과 로그인 Rate Limit을 위한 단일 ElastiCache for Redis OSS 7.1 Stack이다. 기존 `team2` VPC의 Data Subnet 두 개를 사용하며 EKS와 SSM 관리 EC2에서만 TLS `6379` 접근을 허용한다.
...
- User ID: `doro-erp-dev-alpha-default`
- User name: `default`
- Engine: `Redis`
- Authentication: Password
- Access string: `on ~* &* +@all`
- Tags: `Team=team2`, `Project=Doro-ERP`, `Environment=dev`, `Cell=alpha`, `ManagedBy=console-secret-bootstrap`

강한 비밀번호를 생성해 `doro-erp/dev/alpha/store-access` Secret의 `STORE_ACCESS_REDIS_USERNAME=default`, `STORE_ACCESS_REDIS_PASSWORD=<비밀번호>`에 저장한다. 비밀번호를 `terraform.tfvars`, Shell History, Git에 기록하지 않는다.
...
```bash
cd ~/Doro-ERP-Infra/terraform/environments/dev/redis
...
aws elasticache describe-replication-groups \
  --replication-group-id doro-erp-dev-alpha-session \
